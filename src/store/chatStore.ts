...
export const useChatStore = create<ChatState>((set, get) => ({
    messages: persisted?.messages
        ? sanitizeMessages(persisted.messages)
        : {},
    typing: {},
    ...
    markDelivered: (id) =>
        set((s) => {
            const next = {
                messages: updateStatus(s.messages, id, 'delivered'),
            };
            persist(next);
            return next;
        }),
    ...
    markRead: (id) =>
        set((s) => {
            const next = {
                messages: updateStatus(s.messages, id, 'read'),
            };
            persist(next);
            return next;
        }),
    ...
    failMessage: (id) =>
        set((s) => {
            const updated = updateStatus(s.messages, id, 'failed');
...
const persist = (state: Partial<ChatState>) => {
    saveState({
        ...
        offlineQueue:
            state.offlineQueue ??
            useChatStore.getState().offlineQueue,
    });
};
